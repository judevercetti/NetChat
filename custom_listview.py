...
class QCustomQWidget (QWidget):
    def __init__ (self, parent = None):
        ...
        self.allQHBoxLayout.addWidget(QPushButton("hello"))
...
class ChatRoomListWidget(QWidget):
    def __init__(self, msgtext, msgtime):
        super().__init__()
...
class exampleQMainWindow (QMainWindow):
    def __init__ (self, listwidget):
        ...
        # Create QListWidget
        self.myQListWidget = listwidget # QListWidget(self)
        for index, name, icon in [
            ('No.1', 'Meyoko',  'icon.png'),
            ('No.2', 'Nyaruko', 'icon.png'),
            ('No.3', 'Louise',  'icon.png')]:
            # Create QCustomQWidget
            myQCustomQWidget = ChatRoomListWidget(name, index)
